...
(function( root, window, document, factory, undefined) {
    ...
    TavoCalendar.prototype.mount = function() {
        ...
        for (var d = 1; d <= days_in_month; d++) {
            ...
            //Lock days
            if ((moment_copy.isBefore(moment(this.state.date_start, this.config.format), "day") && !this.config.past_select) || (moment_copy.isAfter(moment(this.state.date_end, this.config.format), "day") && !this.config.future_select)
            		) {
                day_wrapper_el.className =  day_wrapper_el.className  + " " + CLASS_CALENDAR_DAY_OFF + " " + CLASS_CALENDAR_DAY_LOCK; 
            }
